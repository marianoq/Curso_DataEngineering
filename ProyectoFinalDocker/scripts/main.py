    ...
    try:
        with engine.connect() as connection:
            ...
            CREATE TABLE IF NOT EXISTS quirogamariano_coderhouse.top_tracks (
                ...
                PRIMARY KEY (spotify_id, create_date)
            )
            """
    ...
    try:
        ...
        for index, track in enumerate(top_tracks['items'], start=1):
            if 'track' in track and 'name' in track['track'] and 'artists' in track['track']:
                ...
                top_tracks_data.append((artist_name, track_name, album_name, top_position, track_popularity, artist_genre, playlist_count, create_date, track_id))

        df = pd.DataFrame(top_tracks_data, columns=['artist_name', 'track_name', 'album_name', 'top_position', 'track_popularity', 'artist_genre', 'playlist_count', 'create_date', 'spotify_id'])
        return df
            ...
            for _, row in df.iterrows():
                try:
                    existing_id = connection.execute(
                        text("SELECT id FROM quirogamariano_coderhouse.top_tracks WHERE spotify_id = :spotify_id AND create_date = :create_date"),
                        {'spotify_id': row['spotify_id'], 'create_date': row['create_date']}
                    ).fetchone()

                    if existing_id:
                        logging.info(f"ID de Spotify {row['spotify_id']} ya existe en la base de datos para la fecha {row['create_date']}. Dato no insertado.")
                        continue

                    insert_query = text("""
                    INSERT INTO quirogamariano_coderhouse.top_tracks 
                    (artist_name, track_name, album_name, top_position, track_popularity, artist_genre, playlist_count, create_date, spotify_id)
                    VALUES (:artist_name, :track_name, :album_name, :top_position, :track_popularity, :artist_genre, :playlist_count, :create_date, :spotify_id)
                    """)
